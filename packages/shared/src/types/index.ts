// Enhanced data models for Intelligent Engineering Platform 2.0
// Preserving existing structure while adding collaborative features

// Base types from existing system (preserved exactly)
...
export interface Project {
  // Existing core structure preserved exactly
  id: string;
  name: string;
  requirements: string;
  constraints: string;
...
// Additional collaboration interfaces
...
// EDA/Schematic Integration Types
export type EDASoftwareType = 
  | 'altium' 
  | 'eagle' 
  | 'kicad' 
  | 'orcad' 
...
export interface EDAConfiguration {
  connectionId: string;
  software: EDASoftware;
  credentials: CADCredentials; // Reuse CAD credentials structure
  syncSettings: CADSyncSettings; // Reuse CAD sync settings
  apiEndpoint?: string;
  webhookUrl?: string;
  pluginVersion?: string;
}
...
export type SimulationSoftwareType = 
  | 'ansys' 
  | 'matlab' 
  | 'simulink' 
  | 'comsol' 
